# Lượt chạy 13 — golden set 20 case

- Thời điểm: 2026-07-30T15:04:08.671Z
...
| C03 | ② | tóm tắt | 5 | deck | true | [1,2,3,4,5,10,15,18,22,23,26,27,28] | ⚠️ sufficient: cần false, được true; không hỏi lại để chốt phạm vi |
...
| C07 | ④ | tóm tắt cho tôi nội dung của bài này, chỉ ra | 7 | deck | true | [1,2,3,10,13,14,15,17,18,19,20,21,22,23,24,26,27] | ✅ pass |
...
| C11 | thường | tóm tắt cả bộ slide | 1 | deck | true | [2,3,4,5,10,14,15,18,23,26,27,28,29] | ✅ pass |
...
| C16 | thường | tại sao học viên cần hiểu cách LLM hoạt động | 2 | — | — | [] | ⚠️ request_failed: Bạn hỏi hơi nhanh — chờ 5s rồi hỏi tiếp nhé. |
...
- `answer`: "Khóa học này cung cấp nền tảng về AI và LLM, đi từ lịch sử phát triển 70 năm đến cơ chế vận hành của các mô hình ngôn ngữ lớn (LLM). Nội dung chính bao gồm: cách LLM hoạt động (dự đoán token, cơ chế attention, context), quy trình huấn luyện (Pre-training, SFT, RLHF), sự khác biệt giữa LLM và AI Agen"
- `missing`: (rỗng)

### C16 (thường) — tại sao học viên cần hiểu cách LLM hoạt động
- Trang đang mở: 2 · nguồn case: câu hỏi mở về mục tiêu buổi
- Lỗi: request_failed: Bạn hỏi hơi nhanh — chờ 5s rồi hỏi tiếp nhé.
- `answer`: (rỗng)
- `missing`: (rỗng)
